...
def read_posters(wb) -> list[dict]:
    ...
    for row in ws.iter_rows(min_row=1, max_row=ws.max_row):
        ...
        posters.append({"id": f"POST-{n:02d}", "last": last, "first": first, "org": "",
                        "title": new_title, "section": sec, "board": "", "note_ru": note, "note_en": ""})
    return posters
...
def write_workbook(path: Path, talks: list[dict], plenary: list[dict], blocks: list[dict], posters: list[dict]) -> None:
    ...
    style_header(ws, len(COLS_TALKS), [9, 8, 6, 20, 24, 18, 80, 12, 8, 12, 10, 24, 24])
    dv = DataValidation(type="list", formula1='"отменён,перенесён"', allow_blank=True)
    ws.add_data_validation(dv)
    dv.add(f"J2:J{max(ws.max_row, 2) + 300}")
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
    ...
    for p in posters:
        ws.append([p["id"], p["last"], p["first"], p["org"], p["title"], p["section"], p["board"], p["note_ru"], p["note_en"]])
    style_header(ws, len(COLS_POSTERS), [10, 20, 24, 18, 80, 8, 10, 30, 30])
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
